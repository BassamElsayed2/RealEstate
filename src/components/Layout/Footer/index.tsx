  ...
  return (
    <footer className="relative z-10 bg-dark">
      <div className="container mx-auto max-w-8xl pt-14 px-4 sm:px-6 lg:px-0">
        <div className={`flex lg:items-center justify-between items-end lg:gap-11 pb-14 border-b border-white/10 lg:flex-nowrap flex-wrap gap-6 ${isArabic ? 'flex-row-reverse' : ''}`}>
          <p className="text-white text-sm lg:max-w-1/5">
            {t('stayUpdatedWithTheLatestNewsPromotionsAndExclusiveOffers')}
          </p>
          <div className={`flex lg:flex-row flex-col items-center lg:gap-10 gap-3 ${isArabic ? 'flex-row-reverse' : ''}`}>
            <div className={`flex gap-2 lg:order-1 order-2 ${isArabic ? 'flex-row-reverse' : ''}`}>
            ...
            <p className="text-white/40 text-sm lg:max-w-[45%] order-1 lg:order-2">
              {t('bySubscribingYouAgreeToReceiveOurPromotionalEmailsYouCanUnsubscribeAtAnyTime')}
            </p>
          </div>
        ...
        <div className={`flex justify-between md:flex-nowrap flex-wrap items-center py-6 gap-6 ${isArabic ? 'flex-row-reverse' : ''}`}>
          <p className="text-white/40 text-sm ">
            ©2025 Homely -  <Link href="https://getnextjstemplates.com/" className="hover:text-primary" target="_blanck">{t('designedAndDevelopedBy')}</Link>
          </p>
          <div className={`flex gap-8 items-center ${isArabic ? 'flex-row-reverse' : ''}`}>
